    ...
    for line in lines {
        for item in line.items() {
            if let PositionedLayoutItem::GlyphRun(glyph_run) = item {
                ...
                // Styles
                let node = doc.get_node(style.brush.id).unwrap();
                let styles = node.primary_styles().unwrap();
                let itext_styles = styles.get_inherited_text();
                let text_styles = styles.get_text();
                let placeholder_active = node.attrs().is_some_and(|attrs| {
                    attrs
                        .iter()
                        .any(|attr| attr.name.local.as_ref() == "data-ox-placeholder-active")
                });
                let text_color = if placeholder_active {
                    itext_styles.color.as_color_color().multiply_alpha(0.82)
                } else {
                    itext_styles.color.as_color_color()
                };
