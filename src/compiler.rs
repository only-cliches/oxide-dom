...
pub(crate) fn compile_module_source(path: &Path, source: &str) -> Result<String, CompileError> {
...
pub(crate) fn is_compilable_module(path: &Path) -> bool {
    needs_compile(path)
}
...
impl JsxGenerator {
        ...
        for attr in attrs {
            match attr {
                ...
                JsxAttribute::Prop { name, value } if name.starts_with("use:") => {
                    ...
                    result
                        .exprs
                        .insert(0, format!("_ox_use({directive}, {el}, {value});"));
                }
    ...
    fn process_spreads(
        ...
        let props = if spread_args.len() == 1 && !dynamic_spread {
            ...
            format!("_ox_mergeProps({})", spread_args.join(", "))
        };

        (
            filtered,
            Some(format!("_ox_spread({el}, {props}, {has_children});")),
        )
    }
    ...
    fn transform_native_children(
        ...
        for index in 0..child_results.len() {
            ...
            if let Some(id) = &child.id {
                appends.push(format!("_ox_insertNode({el}, {id});"));
                ...
                if multi {
                    result
                        .exprs
                        .push(format!("_ox_insert({el}, {expr}, {anchor});"));
                } else {
                    result.exprs.push(format!("_ox_insert({el}, {expr});"));
                }
            }
        }
    ...
    fn build_child(&mut self, child: &JsxChild, multi: bool) -> ChildBuild {
        match child {
            ...
            JsxChild::Expr(expr) if literal_text_value(&prop_expr(expr)).is_some() => {
                let text = literal_text_value(&prop_expr(expr)).unwrap();
    ...
    fn component_expr(&mut self, element: &JsxElement) -> String {
        ...
        let props = match chunks.len() {
            0 => "{}".to_string(),
            1 if !force_merge => chunks.pop().unwrap(),
            _ => format!("_ox_mergeProps({})", chunks.join(", ")),
        };
        format!(
            "_ox_createComponent({}, {props})",
    ...
    fn native_ref_statement(&mut self, element: &str, expr: &str) -> String {
        ...
        if is_static_ref(&expr, &self.static_bindings) || is_function_expr(&expr) {
            format!("_ox_use({expr}, {element});")
        } else {
            let ref_id = self.ref_var();
            if is_assignable_ref(&expr) {
                format!(
                    "var {ref_id} = {expr}; typeof {ref_id} === \"function\" ? _ox_use({ref_id}, {element}) : {expr} = {element};"
                )
            } else {
                format!(
                    "var {ref_id} = {expr}; typeof {ref_id} === \"function\" && _ox_use({ref_id}, {element});"
                )
            }
        }
    }
...
fn wrapped_dynamic_and_child(expr: &str) -> Option<String> {
    ...
    Some(format!(
        "(() => {{ var _c$ = _ox_memo(() => {condition}); return () => _c$() && {value}; }})()"
    ))
}
...
fn wrapped_dynamic_or_child(expr: &str) -> Option<String> {
    ...
    let condition = memo_condition_expr(predicate);
    Some(format!(
        "(() => {{ var _c$ = _ox_memo(() => {condition}); return () => (_c$() && {value}) || {fallback}; }})()"
    ))
}
...
fn wrapped_dynamic_condition_child(expr: &str) -> Option<String> {
    ...
    Some(format!(
        "(() => {{ var _c$ = _ox_memo(() => {condition}); return () => (_c$() ? {consequent} : {alternate}); }})()"
    ))
}
...
fn child_value_expr(expr: &str) -> String {
    ...
    if !once && is_dynamic_expr(&cleaned) && !is_function_expr(&cleaned) {
        if let Some(callee) = zero_arg_call_callee(&cleaned) {
            format!("_ox_memo({callee})")
        } else {
            format!("_ox_memo(() => {})", component_getter_expr(&cleaned))
        }
    } else {
        cleaned
    }
...
fn wrap_dynamics(dynamics: &[DynamicProp]) -> Vec<String> {
    match dynamics {
        [] => Vec::new(),
        [dynamic] => vec![format!(
            "_ox_effect(_$p => _ox_setProp({}, {}, {}, _$p));",
            dynamic.elem,
        ...
        _ => {
            ...
            for (index, dynamic) in dynamics.iter().enumerate() {
                ...
                statements.push(format!(
                    "{value_id} !== _p$.{prop} && (_p$.{prop} = _ox_setProp({}, {}, {value_id}, _p$.{prop}));",
                    dynamic.elem,
                    js_string(&dynamic.key)
                ));
...
fn js_string(value: &str) -> String {
    serde_json::to_string(value).expect("serialize js string")
}

fn universal_import() -> &'static str {
    r#"import { createElement as _ox_createElement, createTextNode as _ox_createTextNode, insertNode as _ox_insertNode, insert as _ox_insert, setProp as _ox_setProp, spread as _ox_spread, effect as _ox_effect, createComponent as _ox_createComponent, mergeProps as _ox_mergeProps, use as _ox_use, memo as _ox_memo, For as _ox_For } from "oxide-runtime";"#
}
